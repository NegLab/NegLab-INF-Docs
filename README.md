# NegLab INF publications

Static GitHub Pages documentation for publications of the INF subproject of
CRC 1629 NegLaB. The site is generated from BibTeX and has no runtime or build
dependencies.
...
The normal import is incremental: an existing publication page is not
overwritten. The main index and publication manifest are refreshed each time.
...
Every supported publication receives a stable, collision-resistant filename in
`docs/publications/`, derived from its BibTeX key. Non-publication entry types,
such as the sample `@project` record, are ignored. Duplicate keys stop the build
with an error.
...
between BibTeX keys and page filenames.
...
bibliography or publication links are updated.
